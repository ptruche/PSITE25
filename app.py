...
# --- JS: dynamically update header margin when sidebar collapses/expands ---
...
def view_quiz():
    ...
    choice = st.radio("", letters, index=default_idx, format_func=lambda L: row[L], label_visibility="collapsed", key=f"radio_{row['id']}")
    st.session_state.quiz_answers[row["id"]] = str(choice).strip().upper()

    c1, c2, c3, c4 = st.columns([1,2,2,1])
    with c1:
        if st.button("Reveal", key=f"rev_{i}"): st.session_state.quiz_revealed.add(row["id"])
    with c2:
        if st.button("Previous", disabled=(i==0)): st.session_state.quiz_idx = max(0, i-1); st.rerun()
    with c3:
        if st.button("Next", disabled=(i==len(pool)-1)): st.session_state.quiz_idx = min(len(pool)-1, i+1); st.rerun()
    with c4:
        if st.button("Finish"): st.session_state.quiz_finished = True
    ...
    if row["id"] in st.session_state.quiz_revealed:
        ...
        if not st.session_state.get(key, False):
            ...
            if st.session_state.get("quiz_mode") == "spaced": sr_update(row["id"], is_correct)
    ...
    if st.session_state.quiz_finished:
        ...
        correct_n = sum(1 for qid, ans in st.session_state.quiz_answers.items() if qid in revealed_ids and correct_map.get(qid) == ans)
